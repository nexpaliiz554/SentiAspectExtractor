...
import extractor.IndexedWordKeys;
import extractor.Opinion;
import extractor.ParsedText;
...
public abstract class TextExtractor {
	...
	public void init() {
		...
		verbAspectList.initVerbAspectList(OPT.verbAspectWordListPath,OPT.isConsiderVerbAspect);
	...
	public void parseText(ParsedText parse_unit) {
		String text = parse_unit.getText();
		...
		ArrayList<SemanticGraph> graphList = textParser.getGraphList();
		setNodeOriIndexScope(nodeList,tokenIndexOriIndexMap);
		parse_unit.setParseInfo(nodeList,graphList);
	}
	
	private void setNodeOriIndexScope(ArrayList<IndexedWord> nodeList, HashMap<Integer,List<Integer>> tokenIndexOriIndexMap){
		for(IndexedWord node: nodeList) {
			int node_beginPos = node.beginPosition();
			int node_index = textParser.getNodeIndexByBeginPos(node_beginPos);
			List<Integer> oriIndexMapForNode = tokenIndexOriIndexMap.get(node_index);
			node.set(IndexedWordKeys.StartOriIndex.class,oriIndexMapForNode.get(0));
			node.set(IndexedWordKeys.EndOriIndex.class,oriIndexMapForNode.get( oriIndexMapForNode.size()-1 ));
		}
	}
	...
	private void structureOpinion(Opinion op) {
    	...
    	ArrayList<IndexedWord> opinionNodeList_update = new ArrayList<IndexedWord>();
    	SemanticGraph graph = textParser.getGraphByNode(coreOpinionNode);
    	int startTokenIndex_update = NLPRule.findConnectedNodeIndex(coreOpinionNode, graph, true);
    	int endTokenIndex_update = NLPRule.findConnectedNodeIndex(coreOpinionNode, graph, false);
    	startTokenIndex = Math.min(startTokenIndex, startTokenIndex_update);
    	endTokenIndex = Math.max(endTokenIndex, endTokenIndex_update);
    	for(int i=startTokenIndex;i<=endTokenIndex;i++) {
			IndexedWord opinionNode = textParser.getNodeByIndex(i);
			opinionNodeList_update.add(opinionNode);
		}
    	op.setOpinionNodeList(opinionNodeList_update);
	}
	...
	public ArrayList<Aspect> extractForOpinion(Opinion op) {
    	...
    	for(Aspect ap:aspectList) {
 	       ...
 	       ap.setOriIndexArr(aspectOriIndex);
    	}
		return aspectList;
    }
...
//		System.out.println("Dependency Graph:\n " +graph.toString(SemanticGraph.OutputFormat.READABLE));
    	...
    	if( nearestSubj==null || peopleWordList.isPeopleNode(nearestSubj) ) {
    		...
    		boolean have_explicit_object = false;
    		...
    		if( !have_explicit_object && nextNode!=null ) {
    ...
    protected void extendAspectAndAddToList(Opinion op,Aspect ap,ArrayList<Aspect> aspectList) {
    	...
    	for(Aspect conjAspect :conjAspectList) {
    		node = conjAspect.getCoreAspectNode();
        	...
        	else if( NLPRule.isNumber(node) ) {
        		extendNumberCoreAspect(op,conjAspect,aspectList);
        	}
        	else if( NLPRule.isNoun(node) || NLPRule.isDeterminer(node) ) {
    ...
    private void checkCoref(Opinion op,Aspect ap,ArrayList<Aspect> aspectList){
    	...
    	if( corefList!=null && corefList.size()!=0) {
    		for(int i=0;i<corefList.size();i++) {
    			...
    			// 从经验的角度来讲，将过长的指代认为是失败的消解：
    			if( corefNodeList.size()!=0 && corefNodeList.size()<=5) {
    ...
    private void extendNumberCoreAspect(Opinion op,Aspect ap,ArrayList<Aspect> aspectList){
    	IndexedWord numberCore = ap.getCoreAspectNode();
    	SemanticGraph graph = textParser.getGraphByNode(numberCore);
    	Set<IndexedWord> childSet = graph.getChildren(numberCore);
		for(IndexedWord child:childSet) {
			String reln = graph.getEdge(numberCore,child).getRelation().toString();
			if( reln.indexOf("nmod")!=-1 && NLPRule.isNoun(child) ) {
				Aspect nounAspect = new Aspect(child,ap.getReasonForSelection());
				extendNounCoreAspect(op,nounAspect,aspectList);
			}
		}
    }
    ...
    private void extendNounCoreAspect(Opinion op,Aspect ap,ArrayList<Aspect> aspectList){
    	...
    	if( OPT.coreExtendType==0 ) {
    		extended = extendBySemanticCompound(op,ap);
    	}
    ...
    private ArrayList<IndexedWord> extendBySemanticCompound(Opinion op,Aspect ap){
    	...
    	// 缺省设置：提取附近联合的名词作为aspect的范围(不对虚词进行提取)
    	if ( compoundNodeList.size()==0 && !implicitAspectWordList.isImplicitNode(core) ) {
    		compoundNodeList = NLPRule.getCompoundNodeByPosotion(core,graph);
    	}
    	// 将国家视作compound的一种:
    	IndexedWord nationCompound = NLPRule.getNationCompoundNode(core, graph);
    	if( nationCompound!=null ) {
    		nationCompound.setTag("NN_POSS");
    		compoundNodeList.add(nationCompound);
    	}
    	// 将所有格视作compound的一种:
    	IndexedWord possessiveNode = NLPRule.getPossessiveNode(core, graph);
    	if( possessiveNode!=null ) {
    		possessiveNode.setTag("NN_POSS");
    		compoundNodeList.add(possessiveNode);
    	}
		...
		//通过连字符扩展aspect
		int update_startTokenIndex = extendNounByHyphen(op,startTokenIndex,endTokenIndex,graph);
		if(update_startTokenIndex!=startTokenIndex) {
			for (int i = update_startTokenIndex; i<=startTokenIndex; i++) {
		   		 IndexedWord node = textParser.getNodeByIndex(i);
		   		 node.setTag("NN");
		   	}
			startTokenIndex = update_startTokenIndex;
		}
		for(int i=startTokenIndex;i<=endTokenIndex;i++) {
			IndexedWord node = textParser.getNodeByIndex(i);
			chunk.add(node);
    ...
    // 检查是否可通过连字符符号，进一步扩展aspect
    private int extendNounByHyphen(Opinion op, int startTokenIndex, int endTokenIndex, SemanticGraph graph) {
    	int ori_startTokenIndex = startTokenIndex;
    	int update_startTokenIndex = startTokenIndex;
    	IndexedWord startNode = textParser.getNodeByIndex(update_startTokenIndex);
    	update_startTokenIndex = NLPRule.findConnectedNodeIndex(startNode, graph, true);
        boolean validModification = false;
        startNode = textParser.getNodeByIndex(update_startTokenIndex);
        for (int i = update_startTokenIndex+1; i <= endTokenIndex; i++) {
            IndexedWord node = textParser.getNodeByIndex(i);
            String reln = (graph.getEdge(node, startNode) != null) ? graph.getEdge(node, startNode).getRelation().toString() : "";
            if ( reln.equals("amod") ) {
        ...
        if (validModification) {
        	return update_startTokenIndex;
        }else {
        	return ori_startTokenIndex;
        }   
    }
    ...
    private void addAspectToListWithoutDup(Opinion op,Aspect ap,ArrayList<Aspect> aspectList) {
    	// 若 aspect 为指代消解失败的代词，无需进行修剪;
    	if( ap.isImplicitAspect() ) {
    		;
    	}
    	// 若 aspect 为动词aspect，无需进行修剪;
    	else if( NLPRule.isVerb(ap.getCoreAspectNode()) ) {
    		;
    	}
    	// 若 aspect 为明确的名词词组，则需要进行修剪；
    	else{
    		// 修剪 aspect 的范围：
        	...
        	// 最后留下的aspect是否是由合法词汇组成的aspect
        	boolean isIllegalWord = isIllegalWord(ap);
        	if( isIllegalWord ) {
    			return ;
        	}
        	// 单词格式合法，但为虚词，语义不足，无法作为一个aspect输出
	...
	private void trimAspectScope(Opinion op,Aspect ap){
		...
		// 检查 trimedApNodeList 中是否仅包含词性为 "NN_POSS" 的项
		boolean onlyPossessive = true;
		for (IndexedWord node : trimedApNodeList) {
		    if (!node.tag().equals("NN_POSS")) { 
		        onlyPossessive = false;
		        break;  
		    }
		}
		if (onlyPossessive) {
		    trimedApNodeList.clear();
		}
    ...
    protected abstract boolean isIllegalWord(Aspect ap);

	public CoreNLPRules getNLPRule() {
		return NLPRule;
	}
