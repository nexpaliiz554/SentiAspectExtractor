	...
	private String enTextTokenMapping(String text,HashMap<Integer,List<Integer>> oriIndexTokenIndexMap,HashMap<Integer,List<Integer>> tokenIndexOriIndexMap) {
		...
		for(int i=0;i<elems.length;i++) {
			...
			if( elem.equals("'") && NLPRule.isHave(nextElem,abbrArr) ) {
				...
				endPositionArr[i+1] = optimizedText.length();
				i++;
			}
			//检查到以特殊符号为开头,则删除
	    ...
	    for(int i=0;i<beginPositionArr.length;i++) {
	    	...
	    	int endPosition = endPositionArr[i];
	    	int corTokenIndex = -1;
	    	...
	    	for(int j=0;j<beginPositionList.size();j++) {
	    		...
	    		if( beginPosition<ti_endPosition && endPosition > ti_beginPosition ) {
	    			corTokenIndex = j;
	    	...
	    	if( !is_updata ) {
	    		oriIndexTokenIndexMap.put(oriIndex, new ArrayList<Integer>());
	    	}
	    }
	    ...
	    for(int i=0;i<tokenList.size();i++) {
	    	...
	    	int endPosition = endPositionList.get(tokenIndex);
	    	int corOriIndex = -1;
	    	boolean is_updata = false;
	    	...
	    	if( !is_updata ) {
	    		tokenIndexOriIndexMap.put(tokenIndex, new ArrayList<Integer>());
	    	}
	    }
