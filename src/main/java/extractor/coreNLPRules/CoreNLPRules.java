...
public abstract class CoreNLPRules {
	...
	//确保节点之间没有被标点符号间隔
	public boolean hasPunctuationBetween(IndexedWord node, IndexedWord child, SemanticGraph graph, String punctuation) {
	    int start = Math.min(node.index(), child.index());
	    int end = Math.max(node.index(), child.index());
	    for (int i = start + 1; i < end; i++) {
	        IndexedWord wordBetween = graph.getNodeByIndexSafe(i); 
	        if (wordBetween != null && wordBetween.word().equals(punctuation)) {
	            return true;  
	        }
	    }
	    return false;  
	}
	public ArrayList<IndexedWord> getChildCompoundNode(IndexedWord node, SemanticGraph graph) {
		ArrayList<IndexedWord> compoundNodeList = new ArrayList<IndexedWord>();
  		Set<IndexedWord> childSet = graph.getChildren(node);
    	for(IndexedWord child:childSet) {
			String reln = graph.getEdge(node,child).getRelation().toString();
			boolean hasPunctuation = hasPunctuationBetween(node, child, graph, "/");
			if( isCompoundReln(reln) && !hasPunctuation && !compoundNodeList.contains(child) ) {
	...
	public ArrayList<IndexedWord> getCompoundNodeByPosotion(IndexedWord node, SemanticGraph graph){
		ArrayList<IndexedWord> compoundNodeList = new ArrayList<IndexedWord>();
		// 向左方向查找连续的名词
	    IndexedWord leftNode = graph.getNodeByIndexSafe(node.index() - 1);
	    while (leftNode != null && isNoun(leftNode)) {
	    	compoundNodeList.add(leftNode);  
	        leftNode = graph.getNodeByIndexSafe(leftNode.index() - 1);
	    }
	    // 向右方向查找连续的名词
	    IndexedWord rightNode = graph.getNodeByIndexSafe(node.index() + 1);
	    while (rightNode != null && isNoun(rightNode)) {
	    	compoundNodeList.add(rightNode);  
	        rightNode = graph.getNodeByIndexSafe(rightNode.index() + 1);
	    }
        return compoundNodeList;
	}
	public IndexedWord getNationCompoundNode(IndexedWord node, SemanticGraph graph){
		IndexedWord nationCompound = null;
		IndexedWord beforeNode = graph.getNodeByIndexSafe(node.index()-1);
		if(beforeNode!= null && beforeNode.ner()!=null && beforeNode.ner().equals("NATIONALITY")) {
			nationCompound = beforeNode;
		}
		return nationCompound;
	}
	//查找形如：“jukebox's” 的形式
	public IndexedWord getPossessiveNode(IndexedWord node, SemanticGraph graph){
		IndexedWord possessiveNode = null;
		boolean has_poss_body = false;
		Set<IndexedWord> govSet = graph.getParents(node);
    	for(IndexedWord gov:govSet) {
			String reln = graph.getEdge(gov,node).getRelation().toString();
			if( reln.equals("nmod:poss") ) {
				has_poss_body = true;
			}
		}
    	if( has_poss_body ) {
    		return possessiveNode;
    	}
    	IndexedWord afterNode = graph.getNodeByIndexSafe(node.index()+1);
		if(afterNode!= null && afterNode.tag().equals("POS")) {
			possessiveNode = afterNode;
		}
		return possessiveNode;
	}
	public ArrayList<IndexedWord> getNodeListByIndex(int startTokenIndex, int endTokenIndex, SemanticGraph graph){
	...
	public int findConnectedNodeIndex(IndexedWord node, SemanticGraph graph, boolean searchLeft) {
	    int currentIndex = node.index();
	    int step = searchLeft ? -1 : 1;
	    int size = graph.size();
	    int lastValidIndex = currentIndex; // 保存最后一个符合条件的节点索引
	    while (true) {
	        ...
	        // 检查下一个节点是否为 "-"
	        IndexedWord hyphenNode = graph.getNodeByIndex(nextIndex);
	        if (hyphenNode == null || !hyphenNode.word().equals("-")) {
	            break; // 下一个节点不是 "-"，停止搜索
	        }
	        // 跨过 "-"，获取下一个节点
	        int wordIndex = nextIndex + step;
	        if (wordIndex < 1 || wordIndex > size) {
	            break; // 超出图的范围
	        }
	        IndexedWord wordNode = graph.getNodeByIndex(wordIndex);
	        if (wordNode == null || !wordNode.word().matches("\\w+")) {
	            break; // 节点为空或不是单词，停止搜索
	        }
	        // 更新当前索引和最后有效的索引
	        currentIndex = wordIndex;
	        lastValidIndex = wordIndex;
	    }
